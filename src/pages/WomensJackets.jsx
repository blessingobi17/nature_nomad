...
const WomensJackets = () => {
  ...
  const datas = searchResults.filter((item) => item.type === "jackets");
  return (
    <main className="py-8 w-full lg:px-20 px-6 relative ">
      <Popup
      ...
      <div className="flex gap-2 w-full justify-center pb-4 text-sm font-medium">
        <Link to="/">Home</Link>
        <span>/</span>
        <Link to="/womenMenu">Women</Link>
        <span>/</span>
        <Link to="/womensJackets" className="text-gray-500">
          Sweatshirts & Jackets
        </Link>
      </div>
      <h1 className="md:text-5xl text-2xl font-semibold pb-2 font-ptSerif text-center">
        Women's Sweatshirts & Jackets
      </h1>
      <div className="relative w-40 pt-6" ref={dropdownRef}>
        ...
        >
          ...
          <FaChevronDown
            className={`text-xs mt-1 ${toggle ? "rotate-180" : ""}`}
            onClick={() => setToggle((prevState) => !prevState)}
          />
        </span>
        <ul
